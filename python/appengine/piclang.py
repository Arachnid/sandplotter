...
class constant(Curve):
    ...
    def __repr__(self):
        if self.val[0] == self.val[1]:
            return "%.3f" % self.val[0]
        else:
            return "(%.3f, %.3f)" % self.val
...
class boustro(Curve):
    ...
    def __call__(self, t):
        if t < 0.5:
            return self.func(t * 2)
        else:
            return self.func((1 - t) * 2)

    def __repr__(self):
        return "boustro(%r)" % self.func
...
def is_atom(obj):
    return isinstance(obj, (PlatonicCircle, PlatonicLine, int, float, tuple))
